...
nest::RecordablesMap< nest::glif_lif_r_asc_a_cond >
  nest::glif_lif_r_asc_a_cond::recordablesMap_;
...
template<>
void
RecordablesMap< nest::glif_lif_r_asc_a_cond >::create()
{
  insert_( names::V_m, &nest::glif_lif_r_asc_a_cond::get_y_elem_< nest::glif_lif_r_asc_a_cond::State_::V_M > );
}
}
...
extern "C" inline int
nest::glif_lif_r_asc_a_cond_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef nest::glif_lif_r_asc_a_cond::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::glif_lif_r_asc_a_cond& node =
    *( reinterpret_cast< nest::glif_lif_r_asc_a_cond* >( pnode ) );
  ...
  {
    const size_t j = i * S::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR + node.P_.n_ASCurrents_() - 1;
...
nest::glif_lif_r_asc_a_cond::Parameters_::Parameters_()
  : th_inf_(26.5) // in mV
  , G_(4.6951) // in nS
  , E_L_(-77.4) // in mv
  , C_m_(99.182) // in pF
...
nest::glif_lif_r_asc_a_cond::State_::State_( const Parameters_& p )
...
nest::glif_lif_r_asc_a_cond::State_::State_( const State_& s )
{
  y_ = s.y_;
}

nest::glif_lif_r_asc_a_cond::State_& nest::glif_lif_r_asc_a_cond::State_::operator=(
  const State_& s )
{
...
void
nest::glif_lif_r_asc_a_cond::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::glif_lif_r_asc_a_cond::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::glif_lif_r_asc_a_cond::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
  ...
  for ( size_t i = 0;
        i < ( ( y_.size() - State_::NUMBER_OF_FIXED_STATES_ELEMENTS - p.n_ASCurrents_() )
              / State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR );
        ++i )
  {
    dg->push_back( y_[ State_::DG_SYN
      + ( State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR * i + p.n_ASCurrents_() - 1 ) ] );
...
void
nest::glif_lif_r_asc_a_cond::State_::set( const DictionaryDatum& d,
  const Parameters_& p )
{
...
nest::glif_lif_r_asc_a_cond::Buffers_::Buffers_( glif_lif_r_asc_a_cond& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::glif_lif_r_asc_a_cond::Buffers_::Buffers_( const Buffers_& b, glif_lif_r_asc_a_cond& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::glif_lif_r_asc_a_cond::glif_lif_r_asc_a_cond()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::glif_lif_r_asc_a_cond::glif_lif_r_asc_a_cond( const glif_lif_r_asc_a_cond& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::glif_lif_r_asc_a_cond::init_state_( const Node& proto )
{
  const glif_lif_r_asc_a_cond& pr = downcast< glif_lif_r_asc_a_cond >( proto );
  S_ = pr.S_;
}

void
nest::glif_lif_r_asc_a_cond::init_buffers_()
{
  ...
  B_.sys_.function = glif_lif_r_asc_a_cond_dynamics;
...
void
nest::glif_lif_r_asc_a_cond::calibrate()
{
  B_.logger_.init();
  ...
  S_.y_.resize( State_::NUMBER_OF_FIXED_STATES_ELEMENTS
      + P_.n_ASCurrents_() + ( State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR * P_.n_receptors_() ),
    0.0 );
...
void
nest::glif_lif_r_asc_a_cond::update( Time const& origin, const long from, const long to )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    if( V_.t_ref_remaining_ > 0.0)
    {
      ...
      if( V_.t_ref_remaining_ <= 0.0)
      {
        ...
        for(std::size_t a = 0; a < P_.n_ASCurrents_(); ++a)
        {
          S_.y_[ State_::ASC + a ] =  P_.asc_amps_[a] + S_.y_[ State_::ASC + a ];
        }
    ...
    for( size_t i = 0; i < P_.n_receptors_(); i++ )
    {
      ...
      S_.y_[ State_::DG_SYN
          + P_.n_ASCurrents_() - 1 + ( State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR * i ) ] +=
...
nest::port
nest::glif_lif_r_asc_a_cond::handles_test_event( SpikeEvent&,
  rport receptor_type )
{
  if ( receptor_type <= 0
...
void
nest::glif_lif_r_asc_a_cond::handle( DataLoggingRequest& e )
{
